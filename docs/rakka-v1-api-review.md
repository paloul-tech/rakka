...
| `rakka-agent-mcp` | Adapter candidate | Remote MCP servers as durable agent tools: server bindings with operator-declared tool policies, a pinned `rmcp` client (Streamable HTTP by default; child-process transport behind `child-process`), the dispatch executor, and stable refusals. Outside the publishable crate set until it enters it. |
