...
| `rakka-agent-mcp` | Adapter | Remote MCP servers as durable agent tools: server bindings with operator-declared tool policies over a pinned `rmcp` client (Streamable HTTP by default, an off-by-default child-process transport), the dispatch executor, descriptor sync, and stable refusals. |
