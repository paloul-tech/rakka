...
| 17.14 Collector allowlist as defence in depth | `transform/allowlist` `keep_keys` over 30 / 44 / 35 keys; the datapoint rule conditioned `where IsMatch(metric.name, "^rakka\\.agent\\.")`, spans and logs deliberately unconditioned | `collector_allowlist.rs` — bijective against the constants, plus the reach of each rule and the values every retention policy selects on | Met |
