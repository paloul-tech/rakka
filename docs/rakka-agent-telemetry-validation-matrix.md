...
| 17.8 model calls are GenAI `chat` spans | `AgentSegmentOperation::ModelInference` closed by the dispatch pipeline → `chat {profile}`, `CLIENT`, `gen_ai.operation.name`, `gen_ai.provider.name`, `gen_ai.request.model`; failures carry `error.type` and `rakka.error.code`; tokens are never invented (a zero direction is omitted, not written); since gap slice 2 the bounded request the adapter receives carries the run's trace context (`AgentModelRequest::telemetry`), so a provider span can parent on the run's trace | `telemetry_segments.rs::the_real_dispatch_pipeline_closes_its_own_segments`, `otel_span_mapping.rs::the_genai_attributes_are_written_by_the_mapping`, `::a_usage_direction_with_no_evidence_is_omitted_rather_than_written_as_zero`, `agent_metrics.rs::a_direction_the_provider_did_not_report_records_no_sample`, `otel_span_mapping.rs::provider_response_fields_are_written_when_reported_and_omitted_when_not` | **Partial.** `gen_ai.usage.*` is attached to the `effect-dispatch` `CONSUMER` span, not the `chat` span; the provider name is the adapter boundary and the request model is the bounded profile; since Phase 7 slice 7.1 the response model, finish reason, and cached/reasoning tokens have slots (`gen_ai.response.model`, `gen_ai.response.finish_reasons`, `rakka.agent.model.cached_input_tokens`, `rakka.agent.model.reasoning_tokens`), written when the provider reported them; no streaming timing, snapshot digest, or model-call latency instrument — see "Owed" |
...
- **A timer or child wait closes no park segment.** Only a checkpoint park
  ...
  exists. Tracked as [#70](https://github.com/paloul-tech/rakka/issues/70).
...
- **17.8's streaming and latency fields have no slot.** Streaming flag and
  first-chunk timing, the context-snapshot digest, and a model-call latency
  instrument do not exist (response model, finish reason, and
  cached/reasoning tokens do, since Phase 7 slice 7.1; the request's trace
  context, which an adapter parents on, does since gap slice 2);
