...
| 17.13 structured logs carry trace context | `AgentLogEvent`, `allowlist_agent_log`, the `tracing` bridge | `exporter_failure.rs::every_declared_export_signal_is_written` | Met at the record; the bridge's own filter is a deployment decision — see "Delegated" |
