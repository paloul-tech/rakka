...
the member whose definition authorizes it, handed off to a specialist that
finishes it, blocked on a human-owned approval, reviewed in a moderated
conversation, and finally closed — surviving an owner death inside the
handoff and inside the conversation, and replayable from a cursor afterwards.
