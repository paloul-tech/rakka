...
```text
ok  1/12 one sharded run completed with the SDK, subscriber, exporter and flush owned by this binary: 39 spans mapped
ok  2/12 12 distinct convention span names left the binary, mapped from the ungated segment vocabulary
...
ok  5/12 all 39 spans joined the ingress trace, each with its own span id
...
ok  9/12 39 spans and every metric crossed a real OTLP gRPC socket
